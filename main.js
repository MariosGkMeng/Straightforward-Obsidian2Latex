...
var I=Object.create;var g=Object.defineProperty;var L=Object.getOwnPropertyDescriptor;var N=Object.getOwnPropertyNames;var P=Object.getPrototypeOf,R=Object.prototype.hasOwnProperty;var C=(c,f)=>{for(var e in f)g(c,e,{get:f[e],enumerable:!0})},E=(c,f,e,n)=>{if(f&&typeof f=="object"||typeof f=="function")for(let t of N(f))!R.call(c,t)&&t!==e&&g(c,t,{get:()=>f[t],enumerable:!(n=L(f,t))||n.enumerable});return c};var S=(c,f,e)=>(e=c!=null?I(P(c)):{},E(f||!c||!c.__esModule?g(e,"default",{value:c,enumerable:!0}):e,c)),O=c=>E(g({},"__esModule",{value:!0}),c);var j={};C(j,{default:()=>w});module.exports=O(j);var i=require("obsidian"),x=require("child_process"),h=require("electron"),r=S(require("fs")),A=S(require("https")),d=S(require("path"));var y={"converter.py":`# Import packages
import re
import sys
import glob, os
import numpy as np
...
print(stats)`,"src/batTrigger.bat":`@echo off\r
setlocal EnableExtensions\r
\r
set "HERE=%~dp0"\r
set "SCRIPT=%HERE%triggerObsidianNoteOnClick.py"\r
set "LOG=%HERE%batTrigger.log"\r
\r
(\r
  echo ==============================\r
  echo %date% %time%\r
  echo HERE   = "%HERE%"\r
  echo SCRIPT = "%SCRIPT%"\r
  where py\r
  where python\r
  echo.\r
\r
  if not exist "%SCRIPT%" (\r
    echo ERROR: Script not found at "%SCRIPT%"\r
    exit /b 1\r
  )\r
\r
  echo Running:\r
...
def extract_latex_code_blocks(S):
    ...
    return latex_code`,"src/custom_latex_functions.tex":`\\usepackage{tikz}\r
\\usepackage{xcolor}\r
\r
% Command to insert vertical lines and new pages\r
\\newcommand{\\DrawVerticalLines}[1]{%\r
    \\par\\bigskip\r
    \\textbf{Inserting #1 empty pages}\\par\\medskip  % <-- added line\r
\r
    \\DrawVerticalLinesAux{#1}%\r
}\r
\r
\\newcommand{\\DrawVerticalLinesAux}[1]{%\r
    \\ifnum#1>0\r
        \\begin{tikzpicture}[remember picture, overlay]\r
            \\draw[thick] (1cm, 0cm) -- (1cm, -\\textheight); % Vertical line\r
        \\end{tikzpicture}\r
        \\newpage\r
...
\r
% Define a new counter for observations\r
\\newcounter{observation}\r
\r
% Define the Observation environment\r
\\newenvironment{observation}[1][]{%\r
	\\refstepcounter{observation} % Increase counter and make it referenceable\r
	\\par\\noindent\\textbf{Observation \\theobservation.} % Bold heading with number\r
}{\\par}\r
\r
\r
\\newenvironment{coloredGreen}{%\r
  \\begin{mdframed}[backgroundcolor=green,linewidth=0pt]%\r
}{%\r
  \\end{mdframed}%\r
}\r
\r
...
\\usepackage{framed}\r
\r
%\\newenvironment{myquote}\r
%{\\begin{leftbar}\\itshape}\r
%	{\\end{leftbar}}\r
	\r
	\r
\\usepackage{framed} % provides leftbar\r
\r
\\newcommand{\\quoteauthor}{}\r
\r
\\newcounter{quotectr}\r
\r
\\newenvironment{myquote}[1]\r
{%\r
	\\refstepcounter{quotectr}%\r
	\\renewcommand{\\quoteauthor}{#1}%\r
	\\begin{leftbar}\\itshape\r
		\\textbf{Quote~\\thequotectr.}\\quad\r
	}\r
	{%\r
		\\par\\hfill--- \\quoteauthor\r
	\\end{leftbar}\r
}\r
\r
\\newcounter{questionctr}\r
\r
\\newenvironment{question}\r
{%\r
	\\refstepcounter{questionctr}%\r
	\\begin{quote}\\bfseries\r
		\\textbf{Question~\\thequestionctr.}\\quad\r
	}\r
	{%\r
	\\end{quote}	\r
}\r
\r
\r
\\newcommand{\\folderlink}[1]{%\r
	\\href{#1}{\\twemoji{open file folder}}%\r
}\r
...
def final_text_replacements(LATEX, markdown_file):
    ...
    return LATEX`,"src/dataview_parser.py":`import re\r
import os\r
...
# print(escaped_text)
`,"src/essential_latex_functions.tex":`\\usepackage{xparse}\r
\r
% --- Encoding ---\r
...
\r
% --- Symbol icons (pdfLaTeX safe) ---\r
\\usepackage{pifont} % provides \\ding\r
\r
% --- xparse for clean environments ---\r
\\usepackage{xparse}\r
\r
\\makeatletter\r
\r
% ------------------------------------------------\r
% Path to your BAT trigger (define once)\r
% ------------------------------------------------\r
\\newcommand{\\BATTRIGGER}{batTrigger.bat}\r
\r
% ------------------------------------------------\r
% Low-level Windows Launch command\r
% ------------------------------------------------\r
\\NewDocumentCommand{\\launchbuttonwin}{m m m}{%\r
	\\leavevmode\r
	\\pdfstartlink attr{/Border[0 0 0]} user{%\r
		/Subtype/Link\r
		/A << /S /Launch\r
		/Win << /F (\\pdfescapestring{#1})\r
		/P (\\pdfescapestring{#2})\r
		>>\r
		>>\r
	}%\r
	#3%\r
...
\r
% ------------------------------------------------\r
% Environment: shows a small clickable icon\r
% Text below flows normally and can span pages\r
% ------------------------------------------------\r
\\NewDocumentEnvironment{launchblock}{m}\r
{%\r
	\\par\\noindent\r
	% Clickable pencil-like icon\r
	\\launchbuttonwin{\\BATTRIGGER}{#1}{\\ding{46}}%\r
	\\hspace{0.5em}\r
}\r
{%\r
	\\par\\medskip\r
...
main().catch(console.error);`,"src/get_parameters.py":`import os\r
...
\r
    ...
    if not path_vault.endswith(os.sep) and not path_vault.endswith('/'):\r
        path_vault += os.sep\r
    command_note        = path_vault+'\u270DWriting\\\\\u{1F468}\u200D\u{1F4BB}convert_to_latex.md'\r
    \r
    try:\r
...
- #Latex/Command/Use_section/End `,"src/move_images_to_local_folder.py":`#!/usr/bin/env python3
"""
collect_figures.py
...
def get_backlinks(note_name, PARS):
    ...
    return backlinks.get(note_name, [])`,"src/remove_markdown_comment.py":`import re\r
\r
def enum(x):\r
    return enumerate(x)\r
\r
def is_in_table_line(x):\r
    x = x.lstrip().rstrip()\r
    if x.startswith('|') and x.endswith('|'):\r
        ...
        return False\r
\r
def remove_markdown_comments(S):\r
\r
    # UNDER DEVELOPMENT!!!\r
\r
    comment_has_started = False\r
    line_number__of_comment_to_close = 0\r
\r
    return_string = False\r
    if isinstance(S, str):\r
        return_string = True\r
        S = [S]\r
\r
    for i, s in enum(S):\r
        to_delete = []\r
        occurences = [x.start() for x in re.finditer(r'(?<!lcmd)%%(?!lcmd)', s)]\r
        L = len(occurences)\r
        if (L > 0):\r
\r
            if comment_has_started:\r
                idx0 = occurences[0]+2\r
                text_in_comment = s[:idx0]\r
                to_delete.append(text_in_comment)\r
                \r
\r
                for tmp in to_delete:\r
                    s = s.replace(tmp, '')\r
                    \r
                    \r
                occurences = [x.start() for x in re.finditer('%%', s)]\r
                L = len(occurences)\r
                comment_has_started = False\r
                \r
            if L % 2 == 0:\r
                to_delete = []\r
                for j in range(int(L/2)):\r
                    j2 = 2*j\r
                    idx0 = occurences[j2]+2\r
                    idx1 = occurences[j2+1]\r
                    text_in_comment = s[idx0:idx1]\r
                    # to_delete.append('%%'+text_in_comment+'%% ') # \u26A0WARNING-2: Bad programming (sloppy way to remove additional space)                                          \r
                    # to_delete.append(' %%'+text_in_comment+'%% ')                                         \r
                    to_delete.append('%%'+text_in_comment+'%%')                     \r
\r
\r
...
\r
                ...
                comment_has_started = True\r
                to_delete = []\r
\r
                s0 = s # need this, since in the following loop, len(s) changes, but the two lines after the loop operate based on the initial length\r
                if L>1:\r
                    for j in range(int((L-1)/2)):\r
                        j2 = 2*j\r
                        idx0 = occurences[j2]+2\r
                        idx1 = occurences[j2+1]\r
                        text_in_comment = s[idx0:idx1]\r
...
\r
\r
                line_number__of_comment_to_close = i\r
\r
                # if not comment_has_started: comment_has_started = True\r
\r
\r
\r
        else:\r
            if comment_has_started:\r
                # need to clear entire line\r
                s = ''\r
\r
    \r
        S[i] = s\r
\r
    if not return_string:\r
        return S\r
    else:\r
        return S[0]\r
...
chcp 65001 >nul
start "" "obsidian://adv-uri?vault=workTips&filepath=P-Tasks/\u2795/\u2795--SINTEF-MANUFACTURING--own questions&heading=Some section"`};var D={pythonPath:"python",converterPath:"",commandNotePath:""},v="3.12.10",q=`https://www.python.org/ftp/python/${v}/python-${v}-embed-amd64.zip`,F="https://bootstrap.pypa.io/get-pip.py",w=class extends i.Plugin{async onload(){await this.loadSettings(),this.ensurePythonAssets(),this.addCommand({id:"convert-current-note",name:"Convert current note to LaTeX",callback:()=>this.convertCurrentNote()}),this.addCommand({id:"run-converter",name:"Run LaTeX converter (use convert_to_latex.md target)",callback:()=>this.runConverter(null,!1)}),this.addCommand({id:"convert-and-compile-current-note",name:"Convert current note to LaTeX AND compile to PDF",callback:()=>this.convertCurrentNote(!0)}),this.addCommand({id:"convert-and-compile",name:"Convert AND compile to PDF (use convert_to_latex.md target)",callback:()=>this.runConverter(null,!0)}),this.addSettingTab(new k(this.app,this))}async convertCurrentNote(e=!1){let n=this.app.workspace.getActiveFile();if(!n){new i.Notice("No active file open.");return}await this.runConverter(n.basename,e)}getVaultDir(){let e=this.app.vault.adapter;if(!(e instanceof i.FileSystemAdapter))throw new Error("This plugin only supports the Obsidian desktop app.");return e.getBasePath()}getPluginDir(){var e;return d.join(this.getVaultDir(),(e=this.manifest.dir)!=null?e:"")}resolveConverterPath(){return this.settings.converterPath||d.join(this.getPluginDir(),"converter.py")}ensurePythonAssets(){let e;try{e=this.getPluginDir()}catch(t){return}for(let[t,l]of Object.entries(y)){let a=d.join(e,...t.split("/"));try{r.mkdirSync(d.dirname(a),{recursive:!0}),(!r.existsSync(a)||r.readFileSync(a,"utf8")!==l)&&r.writeFileSync(a,l,"utf8")}catch(s){console.error(`[LaTeX Converter] Failed to write bundled file ${t}:`,s)}}let n=d.join(e,"src","note_map.json");if(!r.existsSync(n))try{r.mkdirSync(d.dirname(n),{recursive:!0}),r.writeFileSync(n,`{
}
`,"utf8")}catch(t){console.error("[LaTeX Converter] Failed to seed note_map.json:",t)}}downloadFile(e,n,t=5){return new Promise((l,a)=>{let s=r.createWriteStream(n);A.get(e,o=>{if(o.statusCode&&o.statusCode>=300&&o.statusCode<400&&o.headers.location){s.close(),r.unlink(n,()=>{if(t<=0){a(new Error("Too many redirects"));return}this.downloadFile(o.headers.location,n,t-1).then(l,a)});return}if(o.statusCode!==200){s.close(),r.unlink(n,()=>{}),a(new Error(`Download failed: HTTP ${o.statusCode}`));return}o.pipe(s),s.on("finish",()=>s.close(()=>l()))}).on("error",o=>{r.unlink(n,()=>{}),a(o)})})}runCommand(e,n,t){return new Promise((l,a)=>{let s=(0,x.spawn)(e,n,{cwd:t,windowsHide:!1}),o="",m="";s.stdout.on("data",p=>o+=p.toString()),s.stderr.on("data",p=>m+=p.toString()),s.on("close",p=>l({code:p!=null?p:-1,stdout:o,stderr:m})),s.on("error",a)})}async setupBundledPython(){if(process.platform!=="win32"||process.arch!=="x64"){new i.Notice("Bundled Python setup is only available on 64-bit Windows right now.");return}let e=d.join(this.getPluginDir(),"python-embed"),n=d.join(e,"python.exe");if(r.existsSync(n)){this.settings.pythonPath=n,await this.saveSettings(),new i.Notice("Bundled Python is already set up \u2014 pointed the plugin at it.");return}try{r.mkdirSync(e,{recursive:!0}),new i.Notice("Downloading Python...");let t=d.join(e,"python-embed.zip");await this.downloadFile(q,t),new i.Notice("Extracting Python...");let l=await this.runCommand("powershell",["-NoProfile","-Command",`Expand-Archive -LiteralPath '${t}' -DestinationPath '${e}' -Force`]);if(l.code!==0)throw new Error(`Extraction failed: ${l.stderr}`);r.unlinkSync(t);let a=r.readdirSync(e).find(p=>/^python\d+\._pth$/.test(p));if(a){let p=d.join(e,a),_=r.readFileSync(p,"utf8").replace(/#\s*import site/,"import site");r.writeFileSync(p,_,"utf8")}new i.Notice("Installing pip...");let s=d.join(e,"get-pip.py");await this.downloadFile(F,s);let o=await this.runCommand(n,[s,"--no-warn-script-location"],e);if(o.code!==0)throw new Error(`pip bootstrap failed: ${o.stderr}`);r.unlinkSync(s),new i.Notice("Installing numpy...");let m=await this.runCommand(n,["-m","pip","install","numpy"],e);if(m.code!==0)throw new Error(`numpy install failed: ${m.stderr}`);this.settings.pythonPath=n,await this.saveSettings(),new i.Notice("\u2713 Bundled Python set up! 'Python executable' setting updated automatically.")}catch(t){let l=t instanceof Error?t.message:String(t);new i.Notice(`Python setup failed: ${l}`),console.error("[LaTeX Converter] Bundled Python setup failed:",t)}}async runConverter(e,n=!1){if(!this.settings.commandNotePath){new i.Notice("Set 'Command note path' in the LaTeX Converter settings first.");return}let t=this.resolveConverterPath(),l=d.dirname(t),a=null,s=e;if(e!==null)try{a=r.readFileSync(this.settings.commandNotePath,"utf8");let _=a.replace(/convert_note::.*/,`convert_note:: [[${e}]]`);r.writeFileSync(this.settings.commandNotePath,_,"utf8")}catch(_){let u=_ instanceof Error?_.message:String(_);new i.Notice(`Failed to prepare command note: ${u}`);return}else try{let u=r.readFileSync(this.settings.commandNotePath,"utf8").match(/convert_note::\s*\[\[([^\]]+)\]\]/);u&&(s=u[1])}catch(_){}new i.Notice(`Converting ${s!=null?s:"note"}...`);let o=(0,x.spawn)(this.settings.pythonPath,[t],{cwd:l,windowsHide:!1,env:{...process.env,PYTHONIOENCODING:"utf-8",OBSIDIAN_VAULT_PATH:this.getVaultDir()}}),m="",p="";o.stdout.on("data",_=>{m+=_.toString()}),o.stderr.on("data",_=>{p+=_.toString()}),o.on("close",_=>{if(a!==null)try{r.writeFileSync(this.settings.commandNotePath,a,"utf8")}catch(u){}if(_===0){new i.Notice("\u2713 Conversion complete!");let u=m.split(`
`).find(b=>b.startsWith("TEX_OUTPUT:"));if(u){let b=u.slice(11).trim();n?this.compilePdf(b):h.shell.openPath(b).then(T=>{T&&new i.Notice(`Failed to open ${b}: ${T}`)})}}else new i.Notice(`Converter failed (exit ${_}). See console for details.`),console.error("[LaTeX Converter] stderr:",p)}),o.on("error",_=>{new i.Notice(`Failed to start converter: ${_.message}`)})}compilePdf(e){let n=d.dirname(e);new i.Notice("Compiling PDF...");let t=(0,x.spawn)("latexmk",["-pdf","-interaction=nonstopmode","-synctex=1","-shell-escape","-bibtex",e],{cwd:n,windowsHide:!1,env:{...process.env,PYTHONIOENCODING:"utf-8"}}),l="";t.stderr.on("data",a=>{l+=a.toString()}),t.on("close",a=>{if(a===0){new i.Notice("\u2713 PDF ready!");let s=e.replace(/\.tex$/,".pdf");h.shell.openPath(e).then(()=>h.shell.openPath(s))}else new i.Notice(`PDF compilation failed (exit ${a}). See console for details.`),console.error("[LaTeX Converter] latexmk stderr:",l),h.shell.openPath(e)}),t.on("error",a=>{new i.Notice(`Failed to start latexmk: ${a.message}`),h.shell.openPath(e)})}async loadSettings(){let e=await this.loadData();this.settings=Object.assign({},D,e)}async saveSettings(){await this.saveData(this.settings)}},k=class extends i.PluginSettingTab{constructor(e,n){super(e,n);this.plugin=n}getSettingDefinitions(){return[{name:"Python executable",desc:'Path to python (e.g. "python", "python3", or full path like C:\\Python310\\python.exe)',control:{type:"text",key:"pythonPath",placeholder:"python"}},{name:"Bundled Python (Windows only)",desc:"No Python installed? Download a private, self-contained Python + numpy just for this plugin (~30 MB) and point 'Python executable' above at it automatically. Doesn't touch any system-wide Python install.",render:e=>{e.addButton(n=>n.setButtonText("Set up bundled Python").onClick(async()=>{n.setDisabled(!0).setButtonText("Setting up..."),await this.plugin.setupBundledPython(),n.setDisabled(!1).setButtonText("Set up bundled Python"),this.update()}))}},{name:"converter.py path",desc:"Leave empty to use the converter.py bundled with this plugin. Only set this if you keep converter.py somewhere else.",control:{type:"text",key:"converterPath",placeholder:this.plugin.getPluginDir()+d.sep+"converter.py"}},{name:"Command note path",desc:"Required. Full absolute path to a note in your vault containing a line like 'convert_note:: [[Note Name]]' \u2014 the plugin temporarily points this line at the note being converted before running the converter.",control:{type:"text",key:"commandNotePath",placeholder:"C:\\path\\to\\your-vault\\convert_to_latex.md"}}]}};
