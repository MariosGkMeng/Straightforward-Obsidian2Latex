...
- you can write pure latex commands within the obsidian files and they will not be touched by the translator
...
| `PARS['📂']['tex-file']`         | The path to the tex file with the converted content                                                                                |                                                                                                                                                                                                                                                                                                |
| `PARS['📂']['list_paths_notes']` | PURPOSE: Speed-up the conversion, since when searching the entire vault for embedded notes is time consuming (for massive vaults). | (create that text file in this path as well, otherwise you'll receive an error). ℹ If you have many embedded notes that are yet not in that text file, the **first time** you run the code, it will take time (since the code will have to search into the vault, and create that list itself) |
