...
- `PARS['📂']['list_paths_notes']` (create that text file in this path as well, otherwise you'll receive an error)
	- PURPOSE: Speed-up the conversion, since when searching the entire vault for embedded notes is time consuming (for massive vaults) 
	- ℹ If you have many embedded notes that are yet not in that text file, the **first time** you run the code, it will take time (since the code will have to search into the vault, and create that list itself)
